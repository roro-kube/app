...
pub fn load_tray_icon_size(size: u32) -> Result<Icon, String> {
    ...
    // Decode PNG to RGBA
    let img = load_from_memory(png_bytes).map_err(|e| format!("Failed to decode PNG: {e}"))?;
...
fn create_fallback_icon() -> Result<Icon, String> {
    ...
    Icon::from_rgba(icon_data, 32, 32).map_err(|e| format!("Failed to create fallback icon: {e}"))
}
