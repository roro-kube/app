...
fn test_domain_entity_deserialization() {
    ...
    let Ok(entity): Result<DomainEntity, _> = serde_json::from_str(json) else {
...
fn test_entity_state_variants() {
    ...
    for state in states {
        ...
        let Ok(deserialized): Result<EntityState, _> = serde_json::from_str(&serialized) else {
            panic!("deserialization should succeed");
        };
...
fn test_processing_result_deserialization() {
    ...
    let Ok(result): Result<ProcessingResult, _> = serde_json::from_str(json) else {
        panic!("deserialization should succeed");
    };
...
fn test_processing_context_serialization() {
    ...
    let Ok(serialized) = serde_json::to_string(&context) else {
        panic!("serialization should succeed");
    };
