...
fn test_persistence_error_error_messages() {
    ...
    let errors = vec![
        (
            PersistenceError::Database("Connection timeout".to_string()),
            "Database error",
            "Connection timeout",
        ),
        (
            PersistenceError::NotFound("user-456".to_string()),
            "Not found",
            "user-456",
        ),
        (
            PersistenceError::InvalidInput("Missing required field".to_string()),
            "Invalid input",
            "Missing required field",
        ),
        (
            PersistenceError::Serialization("Invalid UTF-8".to_string()),
            "Serialization error",
            "Invalid UTF-8",
        ),
    ];
